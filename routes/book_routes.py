...
from utils.book_recommendations import add_newbook_to_faiss
import asyncpg
...
async def create_book(new_book: Book,current_user: dict = Depends(get_current_user)):
    faiss_index = await add_newbook_to_faiss(new_book)
    conn = await get_db_connection()
    if not conn:
        raise HTTPException(status_code=500, detail="Database connection failed")
    try:
        book_id = await conn.fetchval(
            'INSERT INTO books (title, author, genre, year_published, summary, faiss_index) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
            new_book.title, new_book.author, new_book.genre, new_book.year_published, new_book.summary, faiss_index
        )
...
async def update_book(id: int, updated_book: Book,current_user: dict = Depends(get_current_user)):
    ...
    try:
        await conn.execute(
            ...
            updated_book.title, updated_book.author, updated_book.genre, updated_book.year_published,updated_book.summary, id
        )
